#!/usr/bin/env python
"""
GPU CSMF Test - Validates GPU implementation and compares with CPU.
Tests correctness, performance, and acceleration.
"""
...
def create_scatter_plots_comparison(W_true, W_gpu, W_cpu, title, filename):
    ...
    W_gpu_n = W_gpu / (np.linalg.norm(W_gpu, axis=0, keepdims=True) + 1e-10)
    W_cpu_n = W_cpu / (np.linalg.norm(W_cpu, axis=0, keepdims=True) + 1e-10)
    
    # Align GPU factors
    corr_mat_gpu = np.abs(W_true_n.T @ W_gpu_n)
    _, col_idx_gpu = linear_sum_assignment(-corr_mat_gpu)
    
    # Align CPU factors
    corr_mat_cpu = np.abs(W_true_n.T @ W_cpu_n)
    _, col_idx_cpu = linear_sum_assignment(-corr_mat_cpu)
    
    # Create side-by-side plots
    fig, axes = plt.subplots(r, 2, figsize=(12, 5*r))
    if r == 1:
        axes = axes.reshape(1, 2)
    
    fig.suptitle(f'{title} - GPU vs CPU Factor Comparison', fontsize=14, fontweight='bold')
    
    for i in range(r):
        gpu_idx = col_idx_gpu[i]
        cpu_idx = col_idx_cpu[i]
        
        # GPU subplot
        ax_gpu = axes[i, 0]
        ax_gpu.scatter(W_true[:, i], W_gpu[:, gpu_idx], alpha=0.6, s=40, edgecolors='k', linewidth=0.5)
        min_val = min(W_true[:, i].min(), W_gpu[:, gpu_idx].min())
        max_val = max(W_true[:, i].max(), W_gpu[:, gpu_idx].max())
        ax_gpu.plot([min_val, max_val], [min_val, max_val], 'r--', lw=2, alpha=0.7, label='Perfect')
        corr_gpu = np.corrcoef(W_true[:, i], W_gpu[:, gpu_idx])[0, 1]
        rmse_gpu = np.sqrt(np.mean((W_true[:, i] - W_gpu[:, gpu_idx])**2))
        ax_gpu.set_xlabel('True Factor', fontsize=9)
        ax_gpu.set_ylabel('GPU Factor', fontsize=9)
        ax_gpu.set_title(f'Factor {i+1} (GPU)\nr={corr_gpu:.3f}, RMSE={rmse_gpu:.3f}', fontsize=10)
        ax_gpu.grid(True, alpha=0.3)
        ax_gpu.legend(fontsize=8)
        
        # CPU subplot
        ax_cpu = axes[i, 1]
        ax_cpu.scatter(W_true[:, i], W_cpu[:, cpu_idx], alpha=0.6, s=40, edgecolors='k', linewidth=0.5)
        min_val = min(W_true[:, i].min(), W_cpu[:, cpu_idx].min())
        max_val = max(W_true[:, i].max(), W_cpu[:, cpu_idx].max())
        ax_cpu.plot([min_val, max_val], [min_val, max_val], 'r--', lw=2, alpha=0.7, label='Perfect')
        corr_cpu = np.corrcoef(W_true[:, i], W_cpu[:, cpu_idx])[0, 1]
        rmse_cpu = np.sqrt(np.mean((W_true[:, i] - W_cpu[:, cpu_idx])**2))
        ax_cpu.set_xlabel('True Factor', fontsize=9)
        ax_cpu.set_ylabel('CPU Factor', fontsize=9)
        ax_cpu.set_title(f'Factor {i+1} (CPU)\nr={corr_cpu:.3f}, RMSE={rmse_cpu:.3f}', fontsize=10)
...
print("\n" + "="*70)
print("TEST: GPU CSMF Implementation")
print("="*70)
...
# Run GPU CSMF
print("\n[2] Running GPU CSMF (100 iterations)...")
start = time.time()
result = gpu_csmf(
    X_list,
    rank_common=3,
...
create_scatter_plots(W_c_true_aligned, W_c_aligned, 'W_c Common', 'outputs/test_gpu_wc_scatter.png')
for k in range(len(X_list)):
    W_s_true_k_aligned, W_s_k_aligned, _ = align_and_scale_factors(W_s_true[k], W_s[k])
    create_scatter_plots(W_s_true_k_aligned, W_s_k_aligned, f'W_s[{k}] Specific', f'outputs/test_gpu_ws{k}_scatter.png')

print("\n" + "="*70)
if corr_wc > 0.97 and all(e < 0.05 for e in recon_errors):
    print("✓ TEST PASSED - GPU CSMF working correctly")
else:
    print("⚠ TEST ISSUES:")
    if corr_wc <= 0.97:
